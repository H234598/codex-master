...
def test_runner_disables_external_pytest_plugin_autoload(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    index, test_id = project(tmp_path, "    assert run() == 1\n")
    store = StatusStore(tmp_path / ".state")
    evidence_runner = runner(tmp_path, store)
    ...
    result = evidence_runner.run(index, test_id, expected_index_digest=index.digest)
