...
class ServerHelpersTest(unittest.TestCase):
    ...
    def test_updated_mcp_startup_timeout_config_updates_or_inserts_value(self) -> None:
        ...
        array_table = "\n".join(
            [
                ...
                'command = "/tmp/codex-master-mcp"',
                "",
                "[[mcp_servers.codex-master-mcp.extra]]",
                'name = "extra"',
            ]
        )
        array_updated, array_changed, _array_previous = updated_mcp_startup_timeout_config(array_table)
        self.assertTrue(array_changed)
        self.assertLess(array_updated.index("startup_timeout_sec = 120"), array_updated.index("[["))

        quoted_header = array_table.replace(
            "[mcp_servers.codex-master-mcp]", '[mcp_servers."codex-master-mcp"]'
        )
        quoted_header_updated, quoted_header_changed, _quoted_header_previous = updated_mcp_startup_timeout_config(
            quoted_header
        )
        self.assertTrue(quoted_header_changed)
        self.assertEqual(quoted_header_updated.count('[mcp_servers."codex-master-mcp"]'), 1)
        self.assertLess(quoted_header_updated.index("startup_timeout_sec = 120"), quoted_header_updated.index("[["))

        from tomllib import loads

        single_quoted_header = array_table.replace(
            "[mcp_servers.codex-master-mcp]", "[mcp_servers.'codex-master-mcp']"
        )
        single_quoted_updated, single_quoted_changed, _single_quoted_previous = (
            updated_mcp_startup_timeout_config(single_quoted_header)
        )
        self.assertTrue(single_quoted_changed)
        self.assertEqual(
            loads(single_quoted_updated)["mcp_servers"]["codex-master-mcp"]["startup_timeout_sec"],
            120,
        )
        ...
        for multiline_literal in (
            "startup_timeout_sec = [\n120,\n]",
            'startup_timeout_sec = """\n120\n"""',
        ):
            multiline_config = existing_low.replace("startup_timeout_sec = 30", multiline_literal)
            multiline_updated, multiline_changed, multiline_previous = updated_mcp_startup_timeout_config(
                multiline_config
            )
            self.assertTrue(multiline_changed)
            self.assertIsNone(multiline_previous)
            self.assertEqual(
    ...
    def test_mcp_tools_list(self) -> None:
        ...
        self.assertIn("agent_selection_preview", names)
    ...
    def test_agent_selection_preview_uses_read_only_shadow_without_private_account_data(self) -> None:
        ...
        self.assertEqual(result["candidate_count"], len(inventory.by_series["a-series"]))
        ...
        self.assertTrue(result["admission"]["planned"])
        self.assertFalse(result["admission"]["executable"])
        serialized = json.dumps(result)
        self.assertNotIn("account_key", serialized)
        for descriptor in inventory.agents.values():
    ...
    def test_agent_selection_preview_uses_current_usage_v2_for_sp1a(self) -> None:
        ...
        reset_weekly = (now + timedelta(days=3)).isoformat()
        ...
        fleet_service = Mock()
        fleet_service.load.return_value = server_module.FleetSnapshot(1, 25, (), ())
        fleet_service.account_gate.return_value = Mock(allowed=True, reason="ready")
        snapshot = {
            "account": "BW_Work",
            "status": "ok",
            ...
            "main": {
                ...
                "windows": [
                    {
                        "name": "5h", "duration_seconds": 18000, "remaining": 81.0,
                        "limit": 100.0, "percent": 81.0, "reset_at": reset_5h,
                    },
                    {
                        "name": "weekly", "duration_seconds": 604800, "remaining": 53.0,
                        "limit": 100.0, "percent": 53.0, "reset_at": reset_weekly,
                    },
                ],
            },
        }
        ...
        self.assertEqual(result["usage"], {
            "account_count": 1,
            "observed_count": 1,
            "sp1a_eligible_count": 1,
            "blocked_count": 0,
        })
        self.assertEqual(result["selection"]["selected"]["agent"], "d1")
        self.assertEqual(result["selection"]["selected"]["band"], "sp1a")
    ...
    def test_agent_selection_preview_enforced_remains_closed_without_reservation(self) -> None:
        ...
        ), patch.object(server_module, "read_codex_usage_snapshot", return_value={}):
            result = server_module.fleet_selection_preview(
                series="a", task_kind="simple", admission_mode="enforced", sp3=True
            )

        self.assertFalse(result["admission"]["planned"])
        self.assertFalse(result["admission"]["executable"])
        self.assertEqual(result["admission"]["reason"], "admission_gate_blocked")
        self.assertEqual(len(result["admission"]["missing_gates"]), 7)
    ...
    def test_ordinal_selector_rejects_empty_series(self) -> None:
        with self.assertRaisesRegex(AgentError, "at least one enabled"):
            ordinal_agent_id("1", ())
    ...
    def test_master_watchdog_status_rejects_service_that_has_not_run(self) -> None:
        timer = {
            "ok": True,
            "properties": {
                "LoadState": "loaded",
    ...
    def test_mcp_tools_list_probe_result_accepts_content_length_after_content_type(self) -> None:
        payload = {
            "jsonrpc": "2.0",
            "id": 2,
            "result": {"tools": [{"name": "master_app_bridge_status"}]},
    ...
    def test_master_release_status_detects_release_drift_without_paths(
        ...
        self.assertEqual(result["expected_tag"], "v0.10.0")
    ...
    def test_agent_status_rejects_raw_log_metadata_for_other_agent(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            raw_dir = root / "raw"
            raw_dir.mkdir()
            foreign = raw_dir / "20260717T070001000000Z-b1.log"
    ...
    def test_read_message_accepts_content_length_after_content_type(self) -> None:
        message = {"jsonrpc": "2.0", "id": 1, "method": "resources/list"}
        body = json.dumps(message).encode("utf-8")
    ...
    def test_changed_legacy_agent_configuration_ignores_stale_published_inventory(self) -> None:
        ...
        with server_module.temporary_agent_inventory(published), patch.dict(
            ...
            inventory = server_module.current_agent_inventory()
            _, is_published = server_module.published_agent_inventory()

        self.assertEqual(inventory.agent_ids, ("legacy",))
    ...
    def test_readonly_fleet_service_cannot_quarantine_or_write(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            fleet_root = state_root / "fleet"
            fleet_root.mkdir(mode=0o700)
            limits = fleet_root / "limits.json"
            limits.write_text("{invalid", encoding="utf-8")
            limits.chmod(0o600)
            with patch.object(server_module, "STATE_ROOT", state_root):
                service = server_module._readonly_fleet_service()
                with self.assertRaisesRegex(ValueError, "invalid_fleet_limits"):
                    service.load()
                with self.assertRaisesRegex(AgentError, "fleet_registry_read_only"):
                    service._io.replace_text(fleet_root / "blocked", "x")
    ...
    def test_private_optional_bytes_reads_complete_bounded_artifact(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            expected = b"artifact-content" * 32
            artifact.write_bytes(expected)
            artifact.chmod(0o600)

            self.assertEqual(
                server_module.fleet_read_optional_private_bytes(
                    artifact, len(expected), "artifact_error"
                ),
                expected,
            )
            with self.assertRaisesRegex(AgentError, "artifact_error"):
                server_module.fleet_read_optional_private_bytes(
                    artifact, len(expected) - 1, "artifact_error"
                )
    ...
    def test_watchdog_effective_idle_ignores_future_session_start(self) -> None:
        result = watchdog_effective_idle(
    ...
    def test_fleet_watchdog_dry_run_skips_lifecycle_lock(self) -> None:
        ...
        call_args = run_agent.call_args
        self.assertEqual(call_args.args, ("a1",))
        self.assertEqual(
            {key: value for key, value in call_args.kwargs.items() if key != "snapshot"},
            {
    ...
    def test_fleet_watchdog_does_not_reuse_release_flag_after_session_restart(self) -> None:
        ...
        self.assertFalse(meta_store["watchdog"]["release_lease_after_action"])
    ...
    def test_fleet_watchdog_releases_watchdog_lease_when_activity_recovers(self) -> None:
        ...
        mock_identity.assert_called_once_with("a1", expected_running=True)
    ...
    def test_fleet_watchdog_ignores_non_boolean_release_flag(self) -> None:
        status = {
            "agent": "a",
            "running": False,
            "lease": {"state": "held", "held_by_this_server": True, "raw_output": "not_returned"},
    ...
    def test_fleet_watchdog_action_none_releases_stale_watchdog_lease(self) -> None:
        ...
        payload = result["results"][0]
        self.assertEqual(payload["watchdog_state"], "no_action")
        self.assertEqual(payload["lease_state"], "unclaimed")
        self.assertFalse(payload["held_by_this_server"])
    ...
    def test_fleet_watchdog_cleans_marker_when_agent_stops_during_report_grace(self) -> None:
        status = {
            "agent": "a",
            "running": False,
            "lease": {"state": "held", "held_by_this_server": True, "raw_output": "not_returned"},
    ...
    def test_codex_usage_reader_prefers_current_snapshot_over_legacy(self) -> None:
        from codex_master import server as server_module

        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir) / "codex-usage"
            current = root / "current"
            legacy = root / "snapshots"
            current.mkdir(parents=True)
            legacy.mkdir()
            (current / "a1.json").write_text(
                json.dumps({"account": "a1", "status": "ok", "captured_at": "2099-01-01T00:00:00+00:00"}),
                encoding="utf-8",
            )
            (legacy / "a1.json").write_text(
                json.dumps({"account": "a1", "status": "blocked", "blocked_until": "2099-01-02T00:00:00+00:00"}),
                encoding="utf-8",
            )
            with patch.dict(os.environ, {"XDG_DATA_HOME": tmpdir}, clear=False):
                snapshot = server_module.read_codex_usage_snapshot("a1")

        self.assertEqual(snapshot["status"], "ok")

    def test_codex_usage_reader_does_not_fallback_after_invalid_current_snapshot(self) -> None:
        from codex_master import server as server_module
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            current.mkdir(parents=True)
            legacy.mkdir()
            (current / "a1.json").write_text("not-json", encoding="utf-8")
            (legacy / "a1.json").write_text(
                json.dumps({"account": "a1", "status": "ok"}),
                encoding="utf-8",
            )
            with patch.dict(os.environ, {"XDG_DATA_HOME": tmpdir}, clear=False):
                with self.assertRaisesRegex(AgentError, "could_not_read_codex_usage_snapshot"):
    ...
    def test_current_codex_usage_windows_are_normalized_without_private_fields(self) -> None:
        from codex_master import server as server_module
        ...
        snapshot = {
            ...
            "backend_used": "app-server",
            "backend_account_id": "private-account-id",
            "backend_user_id": "private-user-id",
            "source_urls": ["https://private.invalid"],
            "stale": False,
            "main": {
                "exhausted": False,
                "limit_reached": None,
                "windows": [
                    {
                        ...
                        "limit": 100.0,
                        "percent": 81.0,
                        "reset_at": "2099-01-01T05:00:00+00:00",
                    },
                    {
                        "name": "weekly",
        ...
        usage = state["usage_v2"]
        self.assertEqual(state["state"], "clear")
        self.assertEqual(usage["schema_version"], 2)
        self.assertEqual([item["window_kind"] for item in usage["limit_windows"]], ["rolling_5h", "fixed"])
        self.assertEqual([item["quantity_value"] for item in usage["limit_windows"]], [81, 53])
        self.assertTrue(all(isinstance(item["quantity_value"], int) for item in usage["limit_windows"]))
        serialized_usage = json.dumps(usage)
        for private_field in ("backend_account_id", "backend_user_id", "source_urls"):
            self.assertNotIn(private_field, serialized_usage)
        for private_value in ("private-account-id", "private-user-id", "https://private.invalid"):
            self.assertNotIn(private_value, serialized_usage)

    def test_current_codex_usage_accepts_absolute_window_counts(self) -> None:
        from codex_master import server as server_module
        ...
        state = server_module._codex_usage_watchdog_state_from_snapshot(snapshot, now=0)
        window = state["usage_v2"]["limit_windows"][0]
        self.assertEqual(window["quantity_value"], 80)
        self.assertEqual(window["capacity"], 10000)
        self.assertEqual(window["absolute_remaining"], 8000)

    def test_current_codex_usage_exhausted_window_blocks_until_reset(self) -> None:
        from codex_master import server as server_module
                ...
                "windows": [
                    {
                        ...
                        "limit": 100.0,
                        "percent": 0.0,
                        "reset_at": "2025-12-31T00:00:00+00:00",
                    }
                ],
            },
        }

        state = server_module._codex_usage_watchdog_state_from_snapshot(snapshot, now=1767225600)
    ...
    def test_codex_usage_watchdog_ignores_old_assignment_without_current_route(self) -> None:
        blocked_old = {
            "account": "BW_Alt",
            "status": "blocked",
            "blocked_until": "2099-06-08T06:50:00+00:00",
        ...
        with patch(
            "codex_master.server.read_meta",
            return_value={"started_at_utc": "2026-07-16T10:00:00+00:00"},
        ), patch(
            "codex_master.server.list_assignments",
            return_value={
                "records": [
    ...
    def test_codex_usage_watchdog_ignores_assignment_with_invalid_timestamp(self) -> None:
        blocked_old = {
            "account": "BW_Alt",
            "status": "blocked",
            "blocked_until": "2099-06-08T06:50:00+00:00",
    ...
    def test_codex_usage_watchdog_ignores_future_assignment(self) -> None:
        blocked_future = {
            ...
            "status": "blocked",
            "blocked_until": "2099-06-08T06:50:00+00:00",
            "blocked_reason": "future account limit",
        }

        def read_snapshot(account: str) -> dict[str, Any]:
            return blocked_future if account == "BW_Future" else {}
    ...
    def test_codex_usage_watchdog_ignores_future_assignment_without_session_timestamp(self) -> None:
        blocked_future = {
            "account": "BW_Future",
            "status": "blocked",
            "blocked_until": "2099-06-08T06:50:00+00:00",
    ...
    def test_codex_usage_watchdog_rejects_empty_snapshot_account(self) -> None:
        with patch("codex_master.server.read_meta", return_value={}), patch(
    ...
    def test_codex_usage_watchdog_rejects_empty_assignment_account(self) -> None:
        with patch(
            "codex_master.server.read_meta", return_value={"routing": {"account": "BW_Neu"}}
        ), patch(
            "codex_master.server.list_assignments",
            return_value={"records": [{"routing": {"account": ""}}]},
    ...
    def test_codex_usage_watchdog_rejects_invalid_metadata_decision(self) -> None:
        with patch(
            "codex_master.server.read_meta",
            return_value={"routing": {"account": "BW_Neu", "decision": []}},
        ), patch("codex_master.server.list_assignments", return_value={"records": []}), patch(
            "codex_master.server.read_codex_usage_snapshot"
        ) as read_snapshot:
            with self.assertRaisesRegex(AgentError, "codex-usage routing metadata is invalid"):
    ...
    def test_codex_usage_watchdog_rejects_empty_metadata_account(self) -> None:
        with patch(
            "codex_master.server.read_meta",
            return_value={"routing": {"account": None}},
        ), patch("codex_master.server.list_assignments", return_value={"records": []}), patch(
            "codex_master.server.read_codex_usage_snapshot"
        ) as read_snapshot:
            with self.assertRaisesRegex(AgentError, "codex-usage snapshot account is invalid"):
    ...
    def test_codex_usage_watchdog_rejects_non_mapping_assignment_route(self) -> None:
        with patch("codex_master.server.read_meta", return_value={}), patch(
            "codex_master.server.list_assignments",
            return_value={"records": [{"routing": []}]},
        ), patch("codex_master.server.read_codex_usage_snapshot") as read_snapshot:
    ...
    def test_codex_usage_watchdog_rejects_invalid_assignment_decision(self) -> None:
        with patch("codex_master.server.read_meta", return_value={}), patch(
            "codex_master.server.list_assignments",
            return_value={"records": [{"routing": {"account": "BW_Neu", "decision": []}}]},
        ), patch("codex_master.server.read_codex_usage_snapshot") as read_snapshot:
            with self.assertRaisesRegex(AgentError, "codex-usage assignment routing metadata is invalid"):
                codex_usage_watchdog_status("a1")
    ...
    @patch("codex_master.server.ensure_state")
    def test_codex_usage_watchdog_rejects_invalid_assignment_decision_in_log(self, _mock_ensure_state) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            assignment_log = root / "assignments.jsonl"
            assignment_log.write_text(
                json.dumps({"agent": "a1", "routing": {"account": "BW_Neu", "decision": "mystery"}}) + "\n",
                encoding="utf-8",
            )
            with patch("codex_master.server.STATE_ROOT", root / "state"), patch(
                "codex_master.server.LOCK_DIR", root / "locks"
    ...
    def test_codex_usage_watchdog_status_fails_closed_on_invalid_marker_type(self) -> None:
        with patch(
            "codex_master.server.read_meta",
            return_value={"codex_usage_watchdog": []},
        ):
            with self.assertRaisesRegex(AgentError, "could_not_read_codex_usage_watchdog_marker"):
                codex_usage_watchdog_status("a")
    ...
    def test_codex_usage_watchdog_status_rejects_snapshot_expiry_without_status(self) -> None:
        with patch("codex_master.server.read_meta", return_value={}), patch(
            "codex_master.server.list_assignments", return_value={"records": []}
        ), patch(
            "codex_master.server.read_codex_usage_snapshot",
            return_value={"account": "a1", "blocked_until": "2099-06-08T06:50:00+00:00"},
        ):
            with self.assertRaisesRegex(AgentError, "could_not_read_codex_usage_snapshot"):
                codex_usage_watchdog_status("a")
    ...
    def test_usage_watchdog_resolves_unmapped_account_before_marking(self) -> None:
        ...
        self.assertEqual(result["usage_watchdog_state"], "blocked_marked")
        remember.assert_called_once_with("a1", "BW_Work")
    ...
    def test_replace_private_bytes_fsyncs_temp_and_parent(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            calls: list[int] = []
            with patch.object(server_module.os, "fsync", side_effect=lambda fd: calls.append(fd)):
                server_module.replace_private_bytes(target, b"durable\n")
    ...
    def test_agent_identity_guard_rejects_managed_process_outside_tmux_pane(self) -> None:
        result = agent_identity_guard(
            True,
            {
                "process_count": 1,
    ...
    def test_stop_agent_keeps_lease_when_kill_succeeds_but_home_process_remains(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            state = root / "state"
            with patch("codex_master.server.STATE_ROOT", state), patch(
...
class CliLifecycleTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        # worker whose inherited CODEX_HOME points at the worker home.
        codex_home_patch = patch.dict("os.environ", {"CODEX_HOME": ""}, clear=False)
...
class AgentPoolManagementTest(unittest.TestCase):
    ...
    def test_agent_pool_spec_validation_errors_do_not_echo_request_values(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            dot_path_spec = self._write_spec_payload(
                tmp,
                {
                    "schema_version": 1,
                    "pool_root": str(pool),
            ...
            dot_path_error = self._pool_validate_error_text(dot_path_spec, pool, 71)
    ...
    def test_agent_pool_install_status_copy_auth_and_destroy_are_data_sparse(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(validation["series"], "not_returned")
            self.assertEqual(validation["series_state"], "set")
            self.assertEqual(validation["aliases"], "not_returned")
            self.assertEqual(validation["aliases_state"], "set")
            ...
            self.assertEqual(dry_run["source_agent"], "not_returned")
            self.assertEqual(dry_run["source_agent_state"], "set")
    ...
    def test_agent_pool_copy_auth_does_not_echo_custom_source_agent(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            spec_path = self._write_spec_payload(tmp, spec)
            (pool / f"{custom_prefix}1").mkdir(parents=True)
            (pool / f"{custom_prefix}2").mkdir()
            (pool / f"{custom_prefix}1" / "auth.json").write_text('{"token":"secret"}\n', encoding="utf-8")
            ...
            payload = json.dumps(result, sort_keys=True)
            self.assertEqual(result["source_agent"], "not_returned")
            self.assertEqual(result["source_agent_state"], "set")
            self.assertEqual(result["target_selector"], "not_returned")
            self.assertEqual(result["target_selector_state"], "set")
            self.assertEqual(result["target_count"], 1)
    ...
    def test_agent_pool_validate_does_not_echo_custom_series_alias_or_auth_names(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            spec = {
                ...
                "aliases": {secret_alias: f"{custom_prefix}2"},
            }
            spec_path = self._write_spec_payload(tmp, spec)
            ...
            self.assertEqual(result["series"], "not_returned")
            self.assertEqual(result["series_state"], "set")
            self.assertEqual(result["alias_count"], 1)
            self.assertEqual(result["aliases"], "not_returned")
            self.assertEqual(result["aliases_state"], "set")
            self.assertEqual(result["authenticated_agent_count"], 1)
            self.assertEqual(result["authenticated_agents"], "not_returned")
            self.assertEqual(result["authenticated_agents_state"], "set")
    ...
    def test_agent_pool_wrapper_quotes_codex_bin_special_chars_as_data(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            child_env.pop("CODEX_AGENT_BIN", None)
            self.assertTrue(result["ok"])
