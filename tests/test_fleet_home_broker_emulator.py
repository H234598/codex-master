...
def bind(p=None, checkpoint=BrokerCheckpoint.CREATE_INTENT):
    return TransactionBinding(
        operation_for_checkpoint(checkpoint),
        T,
        U,
        p or principal(),
        PolicyBinding(7, A),
...
def request(transaction_id=T, request_id=REQUEST_ID, expected_agent=AGENT):
    ...
    return AttestHomeRequest(
        ...
        request_id,
        transaction_id,
        BindingExpectation(expected_agent, 3, 9, 7, A, 4),
    )
...
def opened(state=None, status_value=None, now_ns=1):
    ...
    return open_emulator_transaction(
        state, status_value, attest(status_value), now_ns=now_ns
    )
...
def test_open_transaction_rejects_conflicting_id_and_capacity_overflow():
    ...
    with pytest.raises(ChpbValidationError):
        open_emulator_transaction(
            state,
            stat(checkpoint=BrokerCheckpoint.STAGING_PINNED),
            attest(stat(checkpoint=BrokerCheckpoint.STAGING_PINNED)),
            now_ns=2,
        )
    ...
    for n in range(32):
        ...
        s = TransactionStatus(
            TransactionBinding(
                ChpbTransactionOperation.PROVISION, txid, U, p, PolicyBinding(7, A)
            ),
        ...
        full = open_emulator_transaction(
            full,
            s,
            HomeAttestation(
                s.binding,
...
def test_persist_checkpoint_enforces_binding_transition_and_monotonic_now_ns():
    ...
    state = persist_emulator_checkpoint(
        state,
        T,
        current.binding,
        decision,
        obs(BrokerObjectState.STAGING_EMPTY),
        now_ns=2,
    )
...
def test_recover_each_crash_matrix_row_is_deterministic():
    state = opened()
    step = recover_emulator_transaction(
        state, T, principal(), obs(BrokerObjectState.STAGING_EMPTY), now_ns=2
    )
    assert step.action is BrokerRecoveryAction.PERSIST_CHECKPOINT
    assert (
        step.state.transactions[0].status.checkpoint is BrokerCheckpoint.STAGING_PINNED
    )


@pytest.mark.parametrize(
    "object_state", [BrokerObjectState.STAGING_AND_FINAL, BrokerObjectState.DRIFT]
)
def test_recover_publish_conflicts_blocks_without_persisting_and_replays_deterministically(
    object_state,
):
    initial = stat(
        BrokerCheckpoint.PUBLISH_INTENT,
        obs(BrokerObjectState.STAGING_COMPLETE, BrokerRegistryState.NOT_APPLICABLE, 1),
        1,
    )
    ...
    second = recover_emulator_transaction(
        first.state, T, principal(), conflicting, now_ns=3
    )
...
def test_recover_complete_crash_matrix(
    checkpoint, current, registry, index, total, action, target, status_terminal
):
    status_value = stat(
        checkpoint, obs(current, registry, index), total, terminal=status_terminal
    )
    if status_terminal is None:
        state = opened(status_value=status_value)
    else:
        state = BrokerEmulatorState(
            (EmulatorTransaction(status_value, attest(status_value), 0, 0),), (), 0
        )
    step = recover_emulator_transaction(
        state, T, principal(), obs(current, registry, index), now_ns=2
    )
...
def test_deprovision_emulator_persists_cas_then_finalize_before_cleanup_action():
    ...
    assert (
        cas_intent.state.transactions[0].status.checkpoint
        is BrokerCheckpoint.REGISTRY_CAS_INTENT
    )
    ...
    assert (
        finalize_intent.state.transactions[0].status.checkpoint
        is BrokerCheckpoint.FINALIZE_INTENT
    )
...
def _committed_state():
    ...
    state = persist_emulator_checkpoint(
        state, T, status_value.binding, decision, status_value.observation, now_ns=2
    )
...
def test_same_cache_key_changed_request_kind_returns_request_id_reuse_without_payload():
    ...
    changed_kind = QueryTransactionRequest(
        CHPB_PROTOCOL,
...
@pytest.mark.parametrize(
    "changes", [{"mcs_pair": "c0,c2"}, {"cgroup_dev": 1}, {"cgroup_ino": 2}]
)
def test_foreign_mcs_or_cgroup_replay_is_rejected_before_cache_lookup(changes):
    state, _ = _committed_state()
    first = handle_emulator_message(state, principal(), request(), now_ns=2)
    replay = handle_emulator_message(
        first.state, principal(**changes), request(), now_ns=3
    )
...
@pytest.mark.parametrize(
    "expected_changes", [{"policy_generation": 8}, {"projection_digest": "b" * 64}]
)
def test_policy_generation_and_projection_are_stale_before_request_id_reuse(
    expected_changes,
):
    state, _ = _committed_state()
    first = handle_emulator_message(state, principal(), request(), now_ns=2)
    ...
    changed = dataclasses.replace(
        ...
        expected=BindingExpectation(
            ...
            3,
            9,
            expected_changes.get("policy_generation", 7),
            expected_changes.get("projection_digest", A),
            4,
        ),
    )
...
def test_wrong_principal_fenced_and_stale_generation_precede_lookup():
    ...
    assert (
        handle_emulator_message(
            state, principal(agent_id="bee_2"), request(), now_ns=2
        ).reply.result
        is BrokerResultCode.WRONG_PRINCIPAL
    )
    assert (
        handle_emulator_message(
            state, principal(fencing_epoch=5), request(), now_ns=2
        ).reply.result
        is BrokerResultCode.FENCED
    )
    assert (
        handle_emulator_message(
            state, principal(manifest_generation=8), request(), now_ns=2
        ).reply.result
        is BrokerResultCode.STALE_GENERATION
    )
    assert (
        handle_emulator_message(
            state, principal(unit_generation=10), request(), now_ns=2
        ).reply.result
        is BrokerResultCode.STALE_GENERATION
    )
...
def test_terminal_reconnect_with_new_request_id_returns_same_terminal_fact():
    state, status_value = _committed_state()
    step = handle_emulator_message(
        state, principal(), request(request_id="5" * 32), now_ns=2
    )
    assert step.reply.result is BrokerResultCode.OK
    assert step.reply.transaction == status_value
...
def test_cache_full_rejects_new_entry_but_preserves_existing_replay():
    ...
    for n in range(32):
        rid = f"{n + 10:032x}"
        state = handle_emulator_message(
            state, principal(), request(request_id=rid), now_ns=n + 2
        ).state
    full = handle_emulator_message(
        state, principal(), request(request_id="f" * 32), now_ns=40
    )
    ...
    replay = handle_emulator_message(
        full.state,
        principal(),
        request(request_id="0000000000000000000000000000000a"),
        now_ns=41,
    )
...
def test_equal_inputs_produce_equal_sorted_state_and_reply():
    ...
    assert a == b
    assert tuple(
        tx.status.binding.transaction_id for tx in a.state.transactions
    ) == tuple(sorted(tx.status.binding.transaction_id for tx in a.state.transactions))
