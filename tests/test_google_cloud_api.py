...
def test_urllib_transport_encodes_json_and_decodes_object_response() -> None:
    ...
    transport = google_cloud_api._UrlLibTransport.__new__(google_cloud_api._UrlLibTransport)
    transport._opener = opener

    result = transport.request(
        "POST", "https://example.invalid/resource", {"Accept": "application/json"}, {"item": 1}
    )
...
def test_constructor_rejects_missing_auth_and_creates_a_redacted_client() -> None:
    ...
    api = GoogleCloudApi("private-access-token")

    assert repr(api) == "GoogleCloudApi()"
...
def test_list_enabled_services_returns_only_enabled_service_collection() -> None:
    ...
    assert services == [
        {"name": "projects/123/services/apikeys.googleapis.com"}
    ]
    assert transport.calls[0][0:2] == (
        "GET",
