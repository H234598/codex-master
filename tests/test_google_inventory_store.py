...
def _store(tmp_path: Path) -> tuple[GoogleInventoryStore, Path]:
    ...
    path.write_text(yaml.safe_dump(_legacy_document(), sort_keys=False), encoding="utf-8")
    path.chmod(0o600)
...
def test_failed_transform_leaves_original_byte_equal_and_secret_free(tmp_path: Path) -> None:
    store, path = _store(tmp_path)
    original = path.read_bytes()
...
def test_store_accepts_owner_0755_parent_but_rejects_group_write(tmp_path: Path) -> None:
    store, path = _store(tmp_path)
