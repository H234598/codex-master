...
- Install and uninstall symlink operations must require the install-path parent
  ...
  `CODEX_HOME`. Registering install paths must pass a data-sparse MCP startup
  self-test before registration. `doctor` must warn, without returning changed
  file names, when the installed MCP points at a dirty repo worktree.
